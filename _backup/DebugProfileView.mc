...
class DebugProfileView extends WatchUi.View {
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        var profileOk  = false;
        var naStr      = WatchUi.loadResource(Rez.Strings.LabelNa) as String;
        var yrStr      = WatchUi.loadResource(Rez.Strings.LabelYr) as String;
        var cmStr      = WatchUi.loadResource(Rez.Strings.LabelCm) as String;
        var kgStr      = WatchUi.loadResource(Rez.Strings.LabelKg) as String;
        var ageStr     = naStr;
        var heightStr  = naStr;
        ...
        if (Toybox has :UserProfile) {
            ...
            if (profile != null) {
                ...
                if (hCm != null) { heightStr = (hCm as Number).toString() + " " + cmStr; }

                var by = profile.birthYear;
                if (by != null) {
                    var yr = Time.Gregorian.info(Time.now(), Time.FORMAT_SHORT).year;
                    ageStr = (yr - (by as Number)).toString() + " " + yrStr;
                }
        ...
        var genderLbl   = genderMale
            ? (WatchUi.loadResource(Rez.Strings.LabelMale) as String)
            : (WatchUi.loadResource(Rez.Strings.LabelFemale) as String);
        ...
        dc.drawText(cx, sc * 14 / 260, Graphics.FONT_XTINY,
            WatchUi.loadResource(Rez.Strings.LabelFormula) as String,
        ...
        var wtLbl  = WatchUi.loadResource(Rez.Strings.LabelWt) as String;
        var htLbl  = WatchUi.loadResource(Rez.Strings.LabelHt) as String;
        var ageLbl = WatchUi.loadResource(Rez.Strings.LabelAge) as String;
        var sexLbl = WatchUi.loadResource(Rez.Strings.LabelSex) as String;
        if (row1y > sc * 44 / 260 && row1y < h - 10) {
            dc.drawText(col1x, row1y, Graphics.FONT_XTINY, wtLbl + ": " + weightKg.format("%.1f") + " " + kgStr, jL);
            dc.drawText(col2x, row1y, Graphics.FONT_XTINY, htLbl + ": " + heightStr, jR);
        }
        if (row2y > sc * 44 / 260 && row2y < h - 10) {
            dc.drawText(col1x, row2y, Graphics.FONT_XTINY, ageLbl + ": " + ageStr, jL);
            dc.drawText(col2x, row2y, Graphics.FONT_XTINY, sexLbl + ": " + genderLbl, jR);
        }
        var actScoreLbl = WatchUi.loadResource(Rez.Strings.LabelActivityScore) as String;
        var modLbl      = WatchUi.loadResource(Rez.Strings.LabelMod) as String;
        var vigLbl      = WatchUi.loadResource(Rez.Strings.LabelVig) as String;
        _drawLine(dc, cx, sc * 110 / 260 - s, 0xE0E0E0,
            actScoreLbl + " = " + mod.toString() + " " + modLbl + " + " + vig.toString() + " " + vigLbl, h);
        _drawLine(dc, cx, sc * 126 / 260 - s, 0x546E7A,
            WatchUi.loadResource(Rez.Strings.LabelModVig) as String, h);
        ...
        // ── Формулы ──────────────────────────────────────────
        var goalPrefix = WatchUi.loadResource(Rez.Strings.LabelGoalPrefix) as String;
        var recPrefix  = WatchUi.loadResource(Rez.Strings.LabelRecPrefix) as String;
        var goalLine = goalPrefix + " = " + weightKg.format("%.1f") + "x33+" +
                       genderBonus.toString() + " = " + baseGoal.toString();
        _drawLine(dc, cx, sc * 162 / 260 - s, 0xFFB300, goalLine, h);
        _drawLine(dc, cx, sc * 178 / 260 - s, 0x546E7A,
            WatchUi.loadResource(Rez.Strings.LabelBaseNorm) as String, h);

        var recLine = recPrefix + " = " + baseGoal.toString() + "+" +
                      actBonus.toString() + " = " + recVal.toString();
        _drawLine(dc, cx, sc * 204 / 260 - s, 0x29B6F6, recLine, h);
        _drawLine(dc, cx, sc * 220 / 260 - s, 0x546E7A,
            WatchUi.loadResource(Rez.Strings.LabelBaseActivity) as String, h);
        ...
        _drawLine(dc, cx, sc * 248 / 260 - s, 0x3A3A3A,
            (WatchUi.loadResource(Rez.Strings.LabelActivityUpdated) as String) + timeStr, h);
        var cmpStat = Application.Storage.getValue("_cmpStat");
        var cmpStr  = (cmpStat instanceof String) ? ("CMP: " + (cmpStat as String)) : "CMP: -";
        var cmpColor = (cmpStat instanceof String && (cmpStat as String).equals("ok")) ? 0x00FF00 : 0xFF4444;
        _drawLine(dc, cx, sc * 264 / 260 - s, cmpColor, cmpStr, h + 50);
