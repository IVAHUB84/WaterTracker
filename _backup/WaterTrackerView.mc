...
class WaterTrackerView extends WatchUi.View {
    ...
    private var _selectedSlot as Number = 0;   // выделение кнопками
    private var _hasKeyFocus  as Boolean = false; // true = кнопочный режим
    ...
    function initialize() {
        ...
        _itemLabels = ["-100", "+100", "+250", "+500", "+/-",
            WatchUi.loadResource(Rez.Strings.BtnReset) as String] as Array<String>;
    ...
    function getSelectedSlot()  as Number  { return _selectedSlot; }
    function hasKeyFocus()      as Boolean { return _hasKeyFocus; }
    ...
    function getSelectedItemIdx() as Number {
        return _scrollTop % RIGHT_ITEM_COUNT;
    }
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        var topLabel = recReached
            ? (WatchUi.loadResource(Rez.Strings.LabelDone) as String)
            : (WatchUi.loadResource(Rez.Strings.LabelToday) as String);
        ...
        // GOAL + REC — надпись по левому (с учётом окружности), значение по правому (до разделителя)
        var jLeft       = Graphics.TEXT_JUSTIFY_LEFT | Graphics.TEXT_JUSTIFY_VCENTER;

        var goalValStr  = _fmtNum(goal, units);
        var goalLblStr  = WatchUi.loadResource(Rez.Strings.LabelGoalPrefix) as String;
        var goalY       = h * 51 / 100;
        var goalDy      = goalY - h / 2;
        var goalSafeX   = w / 2 - Math.sqrt((radius * radius - goalDy * goalDy).toFloat()).toNumber() + 8;
        dc.setColor(0x5A5A5A, Graphics.COLOR_TRANSPARENT);
        dc.drawText(goalSafeX, goalY, Graphics.FONT_XTINY, goalLblStr, jLeft);
        dc.setColor(goalColor, Graphics.COLOR_TRANSPARENT);
        dc.drawText(rightX, goalY, Graphics.FONT_SMALL, goalValStr, jRight);

        // REC + предупреждение (если профиль неполный)
        _warnR = 0;
        var recRowY = h * 68 / 100;
        _recY  = recRowY;
        _warnY = recRowY;
        var recValStr  = _fmtNum(rec, units);
        var recLblStr  = WatchUi.loadResource(Rez.Strings.LabelRecPrefix) as String;
        var recDy      = recRowY - h / 2;
        var recSafeX   = w / 2 - Math.sqrt((radius * radius - recDy * recDy).toFloat()).toNumber() + 8;
        if (profileIncomplete) {
            _warnR = 5;
            _warnX = recSafeX.toNumber();
            dc.setColor(0x4A4A4A, Graphics.COLOR_TRANSPARENT);
            dc.drawText(recSafeX, recRowY, Graphics.FONT_XTINY, recLblStr, jLeft);
            dc.drawText(rightX, recRowY, Graphics.FONT_SMALL, recValStr, jRight);
    ...
    private function _drawBtn(
        ...
        var isSelected = (_hasKeyFocus && zone == _selectedSlot);
        var r = 10;
        ...
        if (isActive) {
            ...
            // Выделение кнопками: белая заливка фона + цветной текст
            dc.setColor(0xFFFFFF, Graphics.COLOR_TRANSPARENT);
            dc.fillRoundedRectangle(x - 3, y - 3, w + 6, h + 6, r + 3);
            dc.setColor(baseColor, Graphics.COLOR_TRANSPARENT);
            dc.fillRoundedRectangle(x, y, w, h, r);
            // Стрелка-указатель слева
            var ay = y + h / 2;
            dc.setColor(0xFFFFFF, Graphics.COLOR_TRANSPARENT);
            dc.fillPolygon([[x - 10, ay], [x - 4, ay - 5], [x - 4, ay + 5]]);
            dc.setColor(0xFFFFFF, Graphics.COLOR_TRANSPARENT);
        } else {
            dc.setColor(baseColor, Graphics.COLOR_TRANSPARENT);
            dc.fillRoundedRectangle(x, y, w, h, r);
