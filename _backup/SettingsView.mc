...
function _fmtGoal(ml as Number, units as Number) as String {
    if (units == 0) { return ml.toString() + " " + (WatchUi.loadResource(Rez.Strings.UnitMl) as String); }
...
function _fmtInterval(min as Number) as String {
    ...
    return min.toString() + " " + (WatchUi.loadResource(Rez.Strings.UnitMin) as String);
}
